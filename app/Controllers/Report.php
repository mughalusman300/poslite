...
class Report extends BaseController
{
    ...
    public function itemList(){
        ...
        $totalData = $this->Reportmodel->all_items_count();

        $totalFiltered = $totalData;

        if (empty($this->request->getVar('search')['value'])) {

            $items = $this->Reportmodel->all_items($limit,$start);

        } else {

            $search = trim($this->request->getVar('search')['value']); 
            $items =  $this->Reportmodel->items_search($limit,$start,$search);
            $totalFiltered = $this->Reportmodel->items_search_count($search);
