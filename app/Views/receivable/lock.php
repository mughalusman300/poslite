					...
					<table id="payment_details" class="table table-bordered text-nowrap w-100 mt-2">
					    <thead>
					        <tr>
					            ...
					            <th>Description</th>
					            <th>Amount</th>
					            <?php if(in_array('alter_receivable_lock', $_SESSION['permissions']) ) { ?>
						...
						<?php foreach ($paid as $row) { ?>
						        ...
						        <tr>
						            ...
						            <td>
						                <?= htmlspecialchars($row->receivable_detail_desc) ?> <!-- Escapes special characters for security -->
						            </td>
						            <td>
						                <?= number_format($row->receivable_detail_amount, 2) ?> <!-- Formats amount with two decimal places -->
						                <input type="hidden" 
						                	class="amount" 
							                ...
							                <div>
							                    ...
							                    ?>
							                        <?php if ($row->is_lock): ?>
							                            ...
							                            <!-- | <button type="button" data-receivable_detail_id="<?= $row->receivable_detail_id ?>" class="btn btn-sm btn-outline-theme revert">
							                        ...
							                        <?php elseif ($row->is_rejected): ?>
							                            ...
							                            <!-- | <button type="button" data-receivable_detail_id="<?= $row->receivable_detail_id ?>" class="btn btn-sm btn-outline-theme me-2 revert mt-2"><i class="fas fa-undo"></i> Revert</button> -->
							                            <br><i class="fas fa-undo revert" 
							                            style="color: #1f6bff; cursor: pointer;" 
							                            data-receivable_detail_id="<?= $row->receivable_detail_id ?>"
							                            data-bs-toggle="tooltip" data-bs-placement="top" title="Undo last action"></i>
							                        ...
							                        <?php endif; ?>
							                    <?php else: ?>
							                        <button type="button" data-receivable_detail_id="<?= $row->receivable_detail_id ?>" data-type="lock" class="btn btn-sm btn-outline-theme me-2 update-status"> <i class="fas fa-lock"></i> Lock</button>
							                        <button type="button" data-receivable_detail_id="<?= $row->receivable_detail_id ?>" data-type="reject" class="btn btn-sm btn-outline-theme me-2 update-status"> <i class="fas fa-times"></i> Reject</button>
							                    <?php endif; ?>
							                </div>
							            </td>
							        <?php }?>
				...
				<div class="card-footer bg-none fw-bold">
					<div class="row">
						<div class="col-md-12" style="display: flex;justify-content: end">	
							<a type="button" href="<?= URL?>/receivable" class="btn btn-outline-theme me-2 submit"> <i class="fas fa-fw me-2 fa-arrow-left"></i>Go Back</a>
							<button type="button" data-receivable_id="<?= $receivable_id ?>" class="btn btn-outline-theme me-2 complete" style="display: none;"> <i class="far fa-fw me-2 fa-check-square"></i>Complete</button>
						</div>
					</div>
				</div>
			</div>
