$(document).ready(function() {
    ...
    $(document).on('click', '.submit', function(e){ 
        var validate = checkValidation('#add_receivable_form');
        if (validate) {
            ...
            if (parseFloat(amount) > parseFloat(amount_to_be_paid)) {
                ...
                Swal.fire('', 'The amount entered cannot exceed the pending receivable amount. You entered a total of '+amount+', but the pending amount is '+amount_to_be_paid+'', 'error');
                $('.receivable_detail_amount').addClass('is-invalid');
                e.preventDefault();
                return false;
            }
        } else {
    ...
    $(document).on('click', '.edit', function(e){ 
        ...
        if(text == 'Edit') {
            ...
            $this = $(this);
            var amount_selector = $this.closest('tr').find('.receivable_detail_amount');
            var receivable_detail_amount = amount_selector.val();
            var old_receivable_detail_amount = amount_selector.data('old_val');

            if (receivable_detail_amount == '' || parseFloat(receivable_detail_amount) == 0) {
                $this.closest('tr').find('.receivable_detail_amount').addClass('is-invalid');
                Swal.fire('Amount Required', 'Please enter a valid amount', 'error');
                return false;
            }

            var amount_to_be_paid = parseFloat($('.pending_amount').val());
            amount_to_be_paid = amount_to_be_paid + parseFloat(old_receivable_detail_amount);
            // amount_to_be_paid = amount_to_be_paid.toFixed(2);

            if (parseFloat(receivable_detail_amount) > parseFloat(amount_to_be_paid)) {
                Swal.fire('', 'The amount entered cannot exceed the pending receivable amount. You can update the amount upto '+amount_to_be_paid+'', 'error');
                $('.receivable_detail_amount').addClass('is-invalid');
                e.preventDefault();
                return false;
            }
            ...
            var receivable_detail_id = $this.closest('tr').find('.receivable_detail_id').val();
            ...
            var receivable_detail_desc = $this.closest('tr').find('.receivable_detail_desc').val();
            ...
            Swal.fire({
              ...
              confirmButtonText: 'Yes, do it!'
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: base + "/receivable/update_payment",
                        type: "POST",
                        data: {
                            receivable_detail_id: receivable_detail_id,
                            payment_date: payment_date,
                            payment_mode_id: payment_mode_id,
                            receivable_detail_desc: receivable_detail_desc,
                            receivable_detail_amount: receivable_detail_amount
                        },        
                        success: function(data) {
                            if (data.success) {
                                ...
                                $this.closest('tr').find('.receivable_detail_desc_text').text(receivable_detail_desc);

                                receivable_detail_amount_text = receivable_detail_amount;
                                receivable_detail_amount_text = new Intl.NumberFormat('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2
                                }).format(receivable_detail_amount_text);

                                $this.closest('tr').find('.receivable_detail_amount_text').text(receivable_detail_amount_text);
                                ...
                                amount_selector.data('old_val', receivable_detail_amount).attr('data-old_val', receivable_detail_amount);
                                $this.closest('tr').find('.amount').val(receivable_detail_amount);
    ...
    $(document).on('click', '.delete', function(){
        ...
        var receivable_detail_id = $(this).data('receivable_detail_id');
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Yes, do it!'
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: base + "/receivable/delete_line",
                    type: "POST",
                    data: {
                        receivable_detail_id: receivable_detail_id,
                    },        
...
function calculateTotalAmount() {
    ...
    $('#add_receivable_form .receivable_detail_amount').each(function() {
        let value = parseFloat($(this).val()) || 0; // Default to 0 if value is NaN
        total += value;
    });
